...
#include<cmath>
using namespace std;
bool isdeadlinemissed(long long x,fstream &co);//this function will return true if process deadline is going to miss if we run that 
...
long long preempt(long long x);//this function will check whether any process can prempt process x or not,if some process
...
//and if no process are ready to execute then it will select process which is available earlier than other process
//and if all process has reached there occurence limit then this function will return -1;
bool resolve(long long x,long long y);//if process y is having higher priority than x this will return true else it will return false
...
long long process_selector(fstream &co){
    long long i=0,minindex;
	 double min;
    for(i=0;i<n;i++){
   	 if(p[i].occur!=-1&&timer>=p[i].available_time&&isdeadlinemissed(i,co)!=true){//we want index of process which satisfies these
   	       //conditions
   	       break;
     }
    }
    if(i!=n){
      minindex=i;
      min=p[i].deadline_time;
      while(i<n){
   		if(p[i].occur!=-1&&timer>=p[i].available_time&&resolve(minindex,i)&&isdeadlinemissed(i,co)!=true){
   			min=p[i].deadline_time;
   			minindex=i;
		}
		   i++;
      }
      return minindex;//this is index of process which is having deadline earlier than other process
    }
    /////if no process is ready then we will execute below procedure
    for(i=0;i<n;i++){
    ...
    while(i<n){
	  ...
	  else if(min==p[i].available_time&&p[i].occur!=-1){//if both process are available at same then we will use reslove function
	  	  if(resolve(minindex,i)){
   	  	     min=p[i].available_time;
   	  	     minindex=i;
   	      }
	  }	
      i++;		 
    }//minindex will be index of process which is available before other process
    return minindex;
}
long long preempt(long long x,fstream &co){
     double temptimer=timer,min,currenttimer;
     long long i,minindex;
     for(i=0;i<n;i++){//we want one index of process to intilaize minindex which satisfies below conditions 
     ...
     if(i==n){//all process which is having higher priority than x has reached there occurence limit or they may not be able to
        //prempt x
     ...
     while(i<n){
	  ...
	  else if(min==p[i].available_time&&p[i].occur!=-1&&can_prempt_and_deadline(i,x,co)){
   	  	   if(resolve(minindex,i)){
   	  	     min=p[i].available_time;
   	  	     minindex=i;
   	       }
     ...
     temptimer=timer;
     timer=p[minindex].available_time;//updated timer to time at which process of index minindex is ready
...
bool can_prempt_and_deadline(long long x,long long y,fstream &co){
     ...
     if(timer+p[y].remaining_time>p[x].available_time&&resolve(y,x)){//here process of index x can prempt process of index y 
...
bool resolve(long long x,long long minindex){
	if(p[x].deadline_time>p[minindex].deadline_time)//we are resolving based on deadlines
	    return true;
	else if(p[x].deadline_time==p[minindex].deadline_time){
		if(p[x].id>p[minindex].id)//if deadlines are same then we are going for id
		    return true;		
	}
	return false;
}
...
int main(){
   ...
   fstream co;
   co.open("EDF-Log.txt",ios::out);
   ofstream cu("EDF-Stats.txt");
   ...
   while(x!=-1){
	  ...
	  for(j=0;j<n;j++){//after updation of timer every process deadline will be checked
	  	if(timer>p[j].deadline_time&&p[j].occur!=-1){//if deadline missess
	  		...
	  		p[j].dead++;//indicator to tell process deadline has missed
   ...
   long long sum=0,temp=0,succes=0;
   double wait=0.0;
   for(i=0;i<n;i++){
   ...
   wait=wait/n;
   cu<<"Number of processes that came into the system: "<<temp<<"\n";
   cu<<"Number of processes that successfully completed: "<<succes<<"\n";
